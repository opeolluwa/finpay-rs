...
fn extract_env<T: FromStr>(key: &str) -> T
where
    <T as FromStr>::Err: std::fmt::Debug,
{
    let value = env::var(key).expect(&format!("Environment variable {} not set", key));
    value
        .parse::<T>()
        .expect(&format!("Failed to parse {} as target type", key))
}
